...
- **Dead-code findings carry their gate severity** (2026-09-23): each dead-code finding may carry an additive optional `effective_severity` field, `error` or `warn`. The value is the rule severity after `overrides[].rules` resolve for the path of the finding (empty catalog groups and dependency overrides resolve for the file that declares them), and `--fail-on-issues` raises `warn` to `error`. The `fallow dead-code` findings gate fails when a finding is `error`; other gates (regression, stale baseline) decide on their own inputs. The `fallow audit` `new-only` gate fails only on introduced findings, so an inherited `error` finding does not fail the audit, and the combined command exits 0 for machine formats. The field is on the `dead-code` / `check` envelope, the `dead-code-grouped` envelope and the dead-code section of `audit` and the combined envelope. It is not in `required`, so `schema_version` does not change. It is not part of finding identity, baseline keys or fingerprints, so a change of rule severity does not make a finding new. SARIF, CodeClimate and `github-annotations` read the field in the direct run and in `fallow report --from`. When the field is absent (JSON from an older version), they use the earlier rule-based level. Policy violations keep their own `severity`. `prop_drilling_chains`, `thin_wrappers` and `duplicate_prop_shapes` never gate a run. Since 2026-09-24 they carry the field with their base rule severity, so `fallow report --from` renders their level without the config. The exit-code check and `--fail-on-issues` still skip them.
- **Non-gating findings have a capped CI level** (2026-09-24): `prop_drilling_chains`, `thin_wrappers` and `duplicate_prop_shapes` never gate the exit code. SARIF therefore shows them at level `warning` for a `warn` and for an `error` rule, and the SARIF rule `defaultConfiguration.level` for `fallow/prop-drilling`, `fallow/thin-wrapper` and `fallow/duplicate-prop-shape` is `warning` (or `none` for `off`). Before, an `error` rule gave SARIF level `error` for a run that passed. CodeClimate and `github-annotations` do not carry these findings. The rule severity still decides if the finding is reported, and the JSON output does not change. If one of these types starts to gate the exit code, its CI level follows the rule again.
- **`fallow report --from` states when it falls back to the config for a level** (2026-09-24): each dead-code finding of a current saved report carries `effective_severity`, so its SARIF, CodeClimate, comment and review level does not depend on the config at render time, also for an `overrides[].rules` split. Two levels still come from the config: the level of a finding without `effective_severity` (a report from an older version), which uses the base rules and not `overrides[].rules`, and the SARIF rule `defaultConfiguration.level`. When no config is found (no `--config` and no config file at `--root`), the default rules set these levels, and `report --from` prints a `note:` line on stderr: one for findings without a saved severity, and one for SARIF rule default levels that differ from a saved finding level. Pass `--config` with the config of the original run to use its rules. When the config sets the rule of a saved finding without `effective_severity` to `off`, the finding renders at SARIF level `warning` and CodeClimate `minor`, where an earlier version stopped with an internal error. The stdout document does not change shape.
...
- **`--fail-on-issues` raises `warn` complexity findings** (2026-09-24): with `--fail-on-issues` (or `--ci`, which sets it), `fallow health` and the bare `fallow` command write `effective_severity: "error"` on a complexity finding whose `complexity-*` rule is `warn`, as the flag already does for dead-code findings. The `health-findings` gate then fails on it, so such a run exits 1 where it exited 0. `fallow audit` keeps its own verdict semantics and ignores `--fail-on-issues` for dead-code and complexity findings. The GitHub Action and the GitLab template do not pass the flag to the CLI, so they are not affected. The JSON shape does not change.
...
- **A config a framework plugin could not use reaches `workspace_diagnostics[]`**: two new open-set kinds record what a plugin read and could not act on, each carrying `plugin` (the plugin as it labels itself), `key` (the config key) and `reason` (a kebab-case token), with the config file in `path` like every other path-shaped diagnostic field, project-root-relative with forward slashes. No prose member: the `message` every kind carries is composed from those three. `plugin-config-unreadable` answers `degrades_analysis: true` and warns on stderr, because a declaration the user wrote did not reach the analysis: its `reason` set is `not-object-literal`, `array-form`, `spread` and `unreadable-entries`, and the Module Federation reader records it for an `exposes` or `remotes` declaration it could not read in full, naming the plugin that owns the config file (`webpack`, `rspack`, `rsbuild`, `vite`, `nextjs`) when the options were read from that plugin's own config and `module-federation` for a standalone `module-federation.config.*`. The `array-form` token reaches `remotes` only: the array form of `exposes` is read. `plugin-effect-not-modeled` answers false and warns on no channel, because nothing the run could measure was lost: its `reason` set is `key-effect-not-modeled` and `config-property-unreadable`, and the Nuxt auto-import gate records it for a `components:` or `imports:` surface that kept its convention entry patterns under `autoImports`, once per config file and surface, only where a pattern was actually retained. The same plugin also records it for a source file that reads `#components` or `#imports` in a way fallow cannot narrow to names, such as a spread of a namespace import: `path` is that file, `key` is the module (`#components` or `#imports`), `reason` is `key-effect-not-modeled`, and every name of the module counts as used. That entry is recorded after import resolution, from the resolved project that a warm graph cache restores, so a warm run carries it too. Both token sets are OPEN, the same tolerate-unknown-values contract `workspace_diagnostics[].kind` documents, and a token a consumer does not recognise means "some reason" rather than an error. One config file can hold two entries under one kind and one path (`exposes` and `remotes` both unreadable), so a consumer keyed on kind plus path must widen to the payload; the stderr dedupe and both registry folds already do. The kinds are recorded by the plugin stage, which runs in the dead-code prelude, so they reach the envelopes whose run includes a dead-code analyze pass (`dead-code` / `check` grouped and ungrouped, `health`, `flags`, `audit` under `dead_code`, the bare combined root) and not a standalone `fallow dupes`, the rule the analysis-stage kinds already follow. Plugin config parsing is not cached, so a warm cache carries them exactly like a cold run, and each run replaces the previous run's set so a fixed config drops out. `fallow list` and the MCP `project_info` tool carry both kinds too, because they run the same plugin stage: see the next bullet. No consumer change is required: the shipped integrations and the MCP tools select on `degrades_analysis` rather than on a kind allowlist, so the GitHub Action and the GitLab template report `plugin-config-unreadable` in their existing aggregated degraded-inputs warning and set their degraded output. That job-summary note now reads "Some findings or scores were computed over less than the whole project, or from an input that did not load", matching the analyze step's own wording, because not every degrading kind is about files. Exit codes are unchanged and no `schema_version` moves, under the open-set exception for `workspace_diagnostics[].kind` documented above.
- **`fallow list` uses the analysis entry points** (a detection accuracy correction, issue #2804): `fallow list` and the MCP `project_info` tool ran each package's plugins on a separate path, with no workspace merge, no `autoImports` gate and no script analysis, and assembled the entry points with their own discovery, so `fallow list --entry-points` could name files the analysis does not treat as entry points and miss ones it does. On a Nuxt project with `autoImports: true`, it listed every component and composable as an entry point, while `fallow dead-code` reported the unreferenced ones as unused files. The listing now runs the analysis plugin and script stage and the analysis entry-point discovery, so `entry_points[]` and `entry_point_count` are the set the analysis uses (deduplicated, as the analysis counts them), narrowed afterwards to a path or changed-file scope; the plugin stage always sees the whole discovery. The listing also records `plugin-config-unreadable` and `plugin-effect-not-modeled` in its `workspace_diagnostics[]`, with the same payload as the analysis envelopes. The array was present only with the workspace listing (`--workspaces` or the default all-sections run); it is now also present, with only these plugin entries, on a `--plugins` or `--entry-points` listing whose plugin stage recorded one, and absent otherwise, so a listing with no plugin problem keeps its shape. The entry-point set can move on any project where the two paths disagreed; no field changes type and no `schema_version` moves.
...
- **Saved baseline files state which command wrote them**: every baseline `--save-baseline` writes carries a top-level `kind` string of `dead-code`, `dupes` or `health`, spelled exactly as the matching envelope root kinds. Treat the token set as CLOSED PER RELEASE: one release writes only these three tokens, and a later release that adds a command that saves a baseline adds a token. That is why `baseline_staleness.saved_by` (below) is documented as an OPEN set: a reader must accept a token from a newer release. The member is additive and the file format is otherwise unchanged, in both directions. A baseline saved by an earlier release carries no `kind` and loads on all three commands exactly as it did, with the keys it carries deciding which format it is. A baseline saved by this release loads on a binary that predates the member, because no baseline format rejects unknown fields; a `kind` value a newer fallow writes loads too, because the member is never read back through the typed structs. The three formats' own key lists deliberately do not include `kind`: it is the one key all three write, so listing it would make every format declare every other one.
